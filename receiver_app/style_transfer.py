...
class ContentDataset(torch.utils.data.Dataset):
    def __init__(self, root_dir, files=None, transform=None):
        ...
        self.files = [
            f
            for f in (os.listdir(root_dir) if files is None else files)
...
def run_style_transfer(
    ...
    while run[0] <= num_steps:
        ...
        def closure():
            ...
            if run[0] % 50 == 0:
                logger.debug(
                    "run #{:_>4} - Style Loss : {:4f} Content Loss: {:4f}".format(
                ...
                if tmp_dir:
                    if not os.path.exists(tmp_dir):
                        os.makedirs(tmp_dir)
                    util.save_image(
                        input_img, "{0}/tmp_{1:0>4}.jpg".format(tmp_dir, run[0])
                    )
...
def run(
    ...
    logger.debug(
        "GPU detected: %s, image size: %s"
        % (str(torch.cuda.is_available()), image_size)
    )
    ...
    content_img_loader = DataLoader(
        content_img_set, batch_size=1, shuffle=False, num_workers=1
    )
    ...
    logger.debug(
        "Time (in seconds) to apply style-transfer to batch of %i images: %f"
